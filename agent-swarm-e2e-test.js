/**
 * 真实Agent蜂群协作端到端测试
 * 直接使用5个Kimi Code API Key创建Agent、组建蜂群、执行任务
 * 输出完全真实，非模拟
 */
...
async function test1_singleAgentConnectivity() {
  log('TEST-1', '══════════════════════════════════════════════════════════', 'cyan');
  log('TEST-1', '测试1: 5个Agent单点连通性（每个Agent独立对话）', 'cyan');
  log('TEST-1', '══════════════════════════════════════════════════════════', 'cyan');
  ...
  for (const agent of AGENTS) {
    ...
    try {
      ...
      log('TEST-1', `${agent.name}: ✅ 连通 | ${latency}ms | ${tokens} tokens`, 'green');
      log('TEST-1', `  回复: ${content.replace(/\n/g, ' ').slice(0, 120)}...`, 'white');
      results.push({ agent: agent.name, status: 'ok', latency, content, tokens });
    } catch (err) {
      log('TEST-1', `${agent.name}: ❌ 失败 | ${err.message}`, 'red');
...
async function test2_streaming() {
  ...
  try {
    ...
    log('TEST-2', `✅ 流式成功 | ${latency}ms | ${chunks} chunks | ${full.length}字符 | 含中文:${hasChinese}`, 'green');
    log('TEST-2', `内容: ${full.slice(0, 150)}${full.length > 150 ? '...' : ''}`, 'white');
    return { status: 'ok', latency, chunks, content: full, hasChinese };
  } catch (err) {
    log('TEST-2', `❌ 流式失败: ${err.message}`, 'red');
...
...` },
  ...
  log('TEST-3', `子任务:\n${subtasksText.slice(0, 300)}...`, 'white');

  // Step 2: 4个工作Agent并行执行
...
你的职责是第${idx + 1}个维度。请提供详细分析（300字左右），用中文输出。`;
  ...
  for (const r of workerResults) {
    if (r.error) {
      log('TEST-3', `${r.agent}: ❌ ${r.error}`, 'red');
    } else {
      successCount++;
      log('TEST-3', `${r.agent}: ✅ | ${r.tokens} tokens | ${r.content.slice(0, 80)}...`, 'green');
    }
  }
...
async function test5_parallelCompetition() {
  ...
  for (const r of results) {
    if (r.status === 'ok') {
      successCount++;
      log('TEST-5', `${r.agent}: ✅ | ${r.tokens} tokens | ${r.content}`, 'green');
    } else {
      log('TEST-5', `${r.agent}: ❌ | ${r.error}`, 'red');
    }
  }
...
async function main() {
  ...
  console.log(`${C.magenta}║     真实Agent蜂群协作端到端测试                                ║${C.reset}`);
  console.log(`${C.magenta}║     5个Kimi Code API Key + kimi-k2.5模型                      ║${C.reset}`);
  console.log(`${C.magenta}╚══════════════════════════════════════════════════════════════╝${C.reset}`);
  ...
  console.log(`${t1Ok === 5 ? C.green : C.yellow}1. 单Agent连通性: ${t1Ok}/5 ✅${C.reset}`);

  const t2Ok = allResults.test2.status === 'ok';
  console.log(`${t2Ok ? C.green : C.red}2. 流式输出: ${t2Ok ? '✅' : '❌'} | ${allResults.test2.chunks || 0} chunks${C.reset}`);
  ...
  console.log(`${t4Ok ? C.green : C.yellow}4. 顺序流水线: ${t4Ok ? '✅ 4步骤完成' : '⚠️ 部分完成'} | 总耗时${allResults.test4.totalTime}ms${C.reset}`);
  ...
  console.log(`${t6Ok ? C.green : C.red}6. 路由分发: ${t6Ok ? '✅' : '❌'} | 选中${allResults.test6.selectedAgent}${C.reset}`);

  // 真实性验证
  console.log(`${C.cyan}──────────────────────────────────────────${C.reset}`);
  console.log(`${C.cyan}真实性验证（所有输出均为Kimi k2.5实时生成）:${C.reset}`);
  console.log(`  - 流式输出含中文: ${allResults.test2.hasChinese ? '✅' : '❌'}`);
  ...
  console.log(`${C.magenta}══════════════════════════════════════════${C.reset}`);
  console.log(`${allOk ? C.green : C.yellow}总体结果: ${allOk ? '✅ 全部测试通过' : '⚠️ 部分测试通过'}${C.reset}`);
  console.log(`${C.magenta}══════════════════════════════════════════${C.reset}`);

  // 保存报告
  const fs = require('fs');
  const reportPath = 'C:\\Users\\一梦\\Documents\\kimi\\workspace\\agent-swarm-test-report.json';
